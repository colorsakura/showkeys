...
export fn wsk_app_init_privileged(app_ptr: *?*c.struct_wsk_app) bool {
    const allocation = c.calloc(1, @sizeOf(c.struct_wsk_app)) orelse {
        _ = c.fprintf(c.stderr, "Failed to allocate app state\n");
        return false;
    };
    const app: *c.struct_wsk_app = @ptrCast(@alignCast(allocation));
    app_ptr.* = app;
...
export fn wsk_app_init(app: *c.struct_wsk_app, argc: c_int, argv: [*c][*c]u8) bool {
    c.wsk_config_init_defaults(&app.config);
    if (!c.wsk_config_parse(&app.config, argc, argv)) {
        return false;
    }
    ...
    if (!c.wsk_input_init(&app.input, app)) {
        return false;
    }

    if (!c.wsk_wayland_init(&app.wayland, app)) {
        return false;
    }
...
export fn wsk_app_run(app: *c.struct_wsk_app) c_int {
    if (app.config.exit_after_parse) {
    ...
    while (app.run) {
        ...
        while (true) {
            if (c.wsk_wayland_flush(&app.wayland) == -1) {
                _ = c.fprintf(c.stderr, "wl_display_flush: %s\n", c.strerror(errnoPtr().*));
                break;
            }
            if (errnoPtr().* != c.EAGAIN) {
                break;
            }
        }
        if (errnoPtr().* != 0 and errnoPtr().* != c.EAGAIN) {
            break;
        }
        ...
        if ((pollfds[1].revents & c.POLLIN) != 0 and c.wsk_wayland_dispatch(&app.wayland, app) == -1) {
            _ = c.fprintf(c.stderr, "wl_display_dispatch: %s\n", c.strerror(errnoPtr().*));
            break;
        }
...
export fn wsk_app_finish(app: ?*c.struct_wsk_app) void {
